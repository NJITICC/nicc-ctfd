...
function loadChals() {
  return CTFd.api.get_challenge_list().then(function(response) {
    ...
    for (let i = 0; i <= challenges.length - 1; i++) {
      ...
      const chaltags = $("<div class='challenge-tags text-center'></div>")

      for (let j = 0; j < chalinfo.tags.length; j++) {
        const tag = "tag-" + chalinfo.tags[j].value.replace(/ /g, "-");
        chalwrap.addClass(tag);
        chaltags.append($("<span class='badge badge-info challenge-tag'>{0}</span> ".format(chalinfo.tags[j].value)));
      }

      chalbutton.append(chalheader);
      chalbutton.append(chalscore);

      if (chalinfo.tags.length > 0)
        chalbutton.append(chaltags);
